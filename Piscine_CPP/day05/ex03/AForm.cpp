...
AForm::AForm(void) : _name(""), _gradeExe(1), _gradeSigned(1), _isSigned(false) { return; }

AForm::~AForm(void) { return; }

AForm::AForm(std::string name, int gradeExe, int gradeSigned) : _name(name), _gradeExe(gradeExe), _gradeSigned(gradeSigned), _isSigned(false) {
...
AForm::AForm(AForm const & rhs) : _name(rhs.getName()), _gradeExe(rhs.getGradeExe()), _gradeSigned(rhs.getGradeSigned()) {
...
std::ostream & operator<<(std::ostream & o, AForm const & rhs) {
    o << rhs.getName() << ", Aform grade < grade executed :"
        << rhs.getGradeExe() << ", grade signed :"
        << rhs.getGradeSigned() << ">"
        << ((rhs.getIsSigned()) ? std::string(" Aform is signed") : std::string("Aform is not signed"));
    return o;
}
