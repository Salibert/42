...
Parser::Parser(void) : exit(false), _indexLine(0){ return; }

Parser::Parser(Parser const &src) : exit(src.exit), _indexLine(src._indexLine) { return; }
...
int Parser::getIndexLine(void) const {
  return this->_indexLine;
}

int Parser::check(std::string & instrucion) {
  ...
  if (regex_match(instrucion, this->regexContent) && instrucion != "exit") {
    return 0;
...
int Parser::lexer(std::string & instrucion) {
	...
	if (info != "" && instrucion != "") {
		...
		if (tmp.type == "int8" && (value <  std::numeric_limits<int8_t>::lowest() || value > std::numeric_limits<int8_t>::max())) { return 3; }
		if (tmp.type == "int16" && (value <  std::numeric_limits<int16_t>::lowest() || value > std::numeric_limits<int16_t>::max())) { return 3; }
		if (tmp.type == "int32" && (value <  std::numeric_limits<int32_t>::lowest() || value > std::numeric_limits<int32_t>::max() )) { return 3; }
